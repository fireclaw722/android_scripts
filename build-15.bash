...
bdevice() {
	...
	# Save Full OTA
	mv ~/android/lineage/lineage-15.0/signed-ota_update.zip ~/builds/$device/full/LOS-14.1-$(date +%Y%m%d)-$releasever-$device.zip
	...
	if ! ./build/tools/releasetools/ota_from_target_files -k ~/.android-certs/releasekey --block -i ~/builds/$device/target_files/LOS-14.1-*-$releasetype-$device.zip ~/android/lineage/lineage-15.0/signed-target_files.zip ~/builds/$device/delta/LOS-14.1-$(date +%Y%m%d)-$releasever-$device.zip ; then
		echo "Creating Incremental OTA failed. Saving target_files anyways."
		# Save target_files
		mv ~/android/lineage/lineage-15.0/signed-target_files.zip ~/builds/$device/target_files/LOS-14.1-$(date +%Y%m%d)-$releasetype-$device.zip
	fi
	...
	mv ~/android/lineage/lineage-15.0/signed-target_files.zip ~/builds/$device/target_files/LOS-14.1-$(date +%Y%m%d)-$releasever-$device.zip
...
setupenv() {
	...
	if [ $gapps -eq 0 ]; then
		...
		sed -r '281 s/CM_BUILDTYPE := [^ ]*/CM_BUILDTYPE := NOGAPPS/' ~/android/lineage/lineage-15.0/vendor/cm/config/common.mk >common.mk
		mv common.mk ~/android/lineage/lineage-15.0/vendor/cm/config/common.mk
	elif [ $gapps -eq 1 ]; then
		releasetype=gapps

		sed -r '281 s/CM_BUILDTYPE := [^ ]*/CM_BUILDTYPE := GAPPS/' ~/android/lineage/lineage-15.0/vendor/cm/config/common.mk >common.mk
		mv common.mk ~/android/lineage/lineage-15.0/vendor/cm/config/common.mk
	fi
