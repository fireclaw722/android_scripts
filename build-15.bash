...
bdevice() {
	...
	if [ $stable -eq 1 ]; then
		./build/tools/releasetools/img_from_target_files signed-target_files.zip ~/builds/$device/img/LOS-15.0-$(date +%Y%m%d)-$device.zip
	fi
	...
	# Save Full OTA
	mv ~/android/lineage/lineage-15.0/signed-ota_update.zip ~/builds/$device/full/LOS-15.0-$(date +%Y%m%d)$releasever-$device.zip

	# Add Full OTA
	cd ~/updater
	echo "Adding full OTA to list"
	FLASK_APP=updater.app flask addrom -f LOS-15.0-$(date +%Y%m%d)$releasever-$device.zip -d $device -v 14.1 -t "$(date "+%Y-%m-%d %H:%M:%S")" -r $releasetype -m $(md5sum ~/builds/$device/full/LOS-15.0-$(date +%Y%m%d)$releasever-$device.zip  | awk '{ print $1 }') -u https://rctest.nt.jwolfweb.net/builds/$device/full/LOS-15.0-$(date +%Y%m%d)$releasever-$device.zip
	cd ~/android/lineage/lineage-15.0

	# Save old target_files
	mv ~/builds/$device/target_files/LOS-15.0-*-$releasetype-$device.zip ~/builds/$device/target_files/archive/
	
	# Save new target_files
	mv ~/android/lineage/lineage-15.0/signed-target_files.zip ~/builds/$device/target_files/LOS-15.0-$(date +%Y%m%d)$releasever-$device.zip
...
if [ $# -gt 0 ]; then
	...
	case $1 in
		...
		addison-stable|athene-stable|oneplus3-stable|victara-stable)
			...
			#sed -r '281 s/CM_BUILDTYPE := [^ ]*/CM_BUILDTYPE := STABLE/' ~/android/lineage/lineage-15.0/vendor/cm/config/common.mk >common.mk
			#mv common.mk ~/android/lineage/lineage-15.0/vendor/cm/config/common.mk
